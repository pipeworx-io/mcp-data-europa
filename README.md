...
`tools/list` at `https://gateway.pipeworx.io/data-europa/mcp` returns the tools in the table
...
```bash
curl -X POST https://gateway.pipeworx.io/v1/tools/data_europa_search \
  -H 'Content-Type: application/json' \
  -d '{"query":"agriculture exports"}'
```

No account needed for the first calls. Inspect any tool: `GET https://gateway.pipeworx.io/v1/tools/data_europa_search`. Find one: `POST https://gateway.pipeworx.io/v1/tools/search_packs` with `{"query":"..."}`.
...
```json
{
  "mcpServers": {
    "data-europa": {
      "command": "npx",
      "args": ["-y", "@pipeworx/mcp-data-europa"]
    }
  }
}
```
...
```bash
npx -y @pipeworx/mcp-data-europa
```
